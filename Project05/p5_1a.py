...
mu_nd = interpolate(Expression('x[1]<(2/3)*w && x[1]>(1/3)*w ? mu_r_nd:mu_l_nd',w=w_nd,mu_l_nd=mu_l_nd,mu_r_nd=mu_r_nd,degree=1),S)
lambda_nd = interpolate(Expression('x[1]<(2/3)*w && x[1]>(1/3)*w ? lambda_r_nd:lambda_l_nd',w=w_nd,lambda_l_nd=lambda_l_nd,lambda_r_nd=lambda_r_nd,degree=1),S)
rho_nd = interpolate(Expression('x[1]<(2/3)*w && x[1]>(1/3)*w ? rho_r/rho_l:1.0',w=w_nd,rho_l=rho_l,rho_r=rho_r,degree=1),S)
...
plt.legend(loc='best')
plt.savefig('1a_disps.png',bbox_inches='tight')
...
np.savetxt('natfreq_1a.txt', np.c_[nat_freq])
